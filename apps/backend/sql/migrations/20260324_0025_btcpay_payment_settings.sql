...
alter table if exists billing_runtime_settings
  ...
  add column if not exists btcpay_usdt_currency_code_test text default 'USDT-TRON',
  ...
  add column if not exists btcpay_usdt_currency_code_live text default 'USDT-TRON';
